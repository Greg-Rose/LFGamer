...
feature 'user creates LFG for game they own' do
  ...
  #   - Specific can only be up to 150 characters long
  ...
  scenario 'specific is too long' do
    sign_in users[0]
    visit game_path(game)
    fill_in "Specifics", with: "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis pi"
    click_button "Look For Group"

    expect(page).to have_content "Specifics is too long"
    expect(page).to have_content "Create LFG"
    expect(page).to_not have_content "Edit LFG"
